...
do
	...
	echo "#SBATCH --mem-per-cpu=4G                    # memory per cpu-core (4G is default)" >> $job_file
	echo "#SBATCH --gres=${cluster_gpu}               # number of gpus per node" >> $job_file
	echo "#SBATCH --time=24:00:00                     # total run time limit (HH:MM:SS)" >> $job_file
